...
const App = {
    ...
    applyLanguage() {
        ...
        Object.keys(langData).forEach(key => {
            ...
            if (el) {
                const tags = ['SPAN', 'P', 'H1', 'H2', 'H3', 'STRONG', 'BUTTON', 'A'];
                if (tags.includes(el.tagName)) {
                    // Check if it has an icon to preserve
                    const icon = el.querySelector('i');
                    if (icon) {
                        // If it has an icon, we might have nested spans (like btn-add-mail)
                        // If the ID is on the button itself, textContent would remove the icon.
                        // But usually we put IDs on the text span.
                        // For buttons like refresh-now-btn which don't have icons, textContent is fine.
