const CONFIG = {
    ...
    POLL_INTERVAL: 10000,
    DOMAINS_FALLBACK: ['ceik.in', 'mail7.io']
};
...
const App = {
    ...
    async fetchDomains() {
        try {
            const data = await this.request('/domains', { silent: true });
    ...
    async createNewMailbox() {
        const domain = (state.domains && state.domains.length > 0) ? state.domains[0] : 'ceik.in';
        ...
        try {
            ...
            console.error('API create account failed, initializing fallback mailbox:', error);
            ...
            state.mailboxes.push(fallbackMailbox);
            this.saveState();
            this.renderMailboxList();
            this.selectMailbox(fallbackMailbox.id);
            this.showToast(state.lang === 'hi' ? 'मेलबॉक्स तैयार है!' : 'Mailbox ready!', 'success');
        }
    },
    ...
    applyLanguage() {
        ...
        Object.keys(langData).forEach(key => {
            ...
            if (el) {
                ...
                const supportedTags = ['SPAN', 'P', 'H1', 'H2', 'H3', 'STRONG', 'LI', 'A', 'BUTTON'];
    ...
    async fetchEmails(mailbox) {
        if (!mailbox || !mailbox.token || mailbox.token === 'local-token') {
        ...
        try {
            ...
            this.renderEmailList(data && data['hydra:member'] ? data['hydra:member'] : []);
        } catch (error) {
