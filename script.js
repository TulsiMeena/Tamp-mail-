...
const translations = {
    en: {
        ...
        'rev1': '"The fastest temp mail service I\'ve used. The multi-mailbox feature is a lifesaver for developers!"',
        'rev2': '"Clean UI and very intuitive. I love the dark mode and the localized Hindi support."',
        ...
        'a2': 'Emails are stored for a limited time on Mail.tm servers, usually until the domain expires or the service clears old data.',
        'q3': 'Can I send emails?',
        'a3': 'No, this is a receive-only service to protect against abuse and spam.',
    ...
    hi: {
        ...
        'rev1': '"सबसे तेज़ अस्थायी ईमेल सेवा जिसका मैंने उपयोग किया है। मल्टी-मेलबॉक्स फीचर डेवलपर्स के लिए बहुत उपयोगी है!"',
        'rev2': '"साफ UI और बहुत सहज। मुझे डार्क मोड और हिंदी सपोर्ट बहुत पसंद आया।"',
...
const App = {
    ...
    renderMailboxList() {
        ...
        list.innerHTML = state.mailboxes.map(box => {
            const escapedAddress = escapeHTML(box.address);
            const escapedId = escapeHTML(box.id);
            return `
                <div class="mailbox-card ${box.id === state.currentMailboxId ? 'active' : ''}" onclick="App.selectMailbox('${escapedId}')">
                    <span class="email-addr">${escapedAddress}</span>
                    <div class="mailbox-actions">
                        <button class="btn-icon btn-sm" onclick="App.copyToClipboard('${escapedAddress}', event)" title="Copy">
                            <i class="fas fa-copy"></i>
                        </button>
                        <button class="btn-icon btn-sm" onclick="App.showQR('${escapedAddress}', event)" title="QR Code">
                            <i class="fas fa-qrcode"></i>
                        </button>
                        <button class="btn-icon btn-sm" onclick="App.deleteMailbox('${escapedId}', event)" title="Delete">
    ...
    applyLanguage() {
        ...
        Object.keys(langData).forEach(key => {
            ...
            if (el) {
                // Prevent resetting active mailbox address in the label
                const currentMailbox = state.mailboxes.find(b => b.id === state.currentMailboxId);
                if (key === 'current-mail-label' && currentMailbox) {
                    return;
                }

                const supportedTags = ['SPAN', 'P', 'H1', 'H2', 'H3', 'STRONG', 'LI', 'A'];
    ...
    renderEmailList(emails) {
        ...
        list.innerHTML = emails.map(email => {
            const escapedId = escapeHTML(email.id);
            const escapedFrom = escapeHTML(email.from.name || email.from.address);
            const escapedSubject = escapeHTML(email.subject);
            return `
                <div class="email-item" onclick="App.viewEmail('${escapedId}')">
                    <div class="from">${escapedFrom}</div>
                    <div class="subj">${escapedSubject}</div>
    ...
    renderAttachments(msg, mailbox) {
        ...
        if (msg.attachments && msg.attachments.length > 0) {
            ...
            list.innerHTML = msg.attachments.map(att => {
                ...
                const escapedMsgId = escapeHTML(msg.id);
                const escapedAttId = escapeHTML(att.id);
                return `
                    <div class="attachment-item">
                        <i class="fas fa-file"></i>
                        <span>${escapedFilename} (${(att.size / 1024).toFixed(1)} KB)</span>
                        <button class="btn-text" onclick="App.downloadAttachment('${escapedMsgId}', '${escapedAttId}', '${escapedFilename}')">Download</button>
                    </div>
                `;
            }).join('');
        } else {
