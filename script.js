...
const App = {
    ...
    renderMailboxList() {
        ...
        state.mailboxes.forEach(box => {
            ...
            card.className = `mailbox-card ${box.id === state.currentMailboxId ? 'active' : ''}`;
            card.onclick = () => this.selectMailbox(box.id);

            const addr = document.createElement('span');
            addr.className = 'email-addr';
            addr.textContent = box.address;

            const actions = document.createElement('div');
            actions.className = 'mailbox-actions';

            const copyBtn = this.createActionButton('fas fa-copy', 'Copy', (e) => this.copyToClipboard(box.address, e));
            const qrBtn = this.createActionButton('fas fa-qrcode', 'QR Code', (e) => this.showQR(box.address, e));
            const delBtn = this.createActionButton('fas fa-trash', 'Delete', (e) => this.deleteMailbox(box.id, e));
    ...
    renderEmailList(emails) {
        ...
        if (emails.length === 0) {
            ...
            `;
            document.getElementById('empty-msg').textContent = translations[state.lang]['empty-msg'];
            return;
        }
        ...
        emails.forEach(email => {
            ...
            item.onclick = () => this.viewEmail(email.id);

            const fromDiv = document.createElement('div');
            fromDiv.className = 'from';
            fromDiv.textContent = email.from.name || email.from.address;

            const subjDiv = document.createElement('div');
            subjDiv.className = 'subj';
            subjDiv.textContent = email.subject;

            const dateDiv = document.createElement('div');
            dateDiv.className = 'date';
            dateDiv.textContent = new Date(email.createdAt).toLocaleTimeString();

            item.appendChild(fromDiv);
            item.appendChild(subjDiv);
